...
- Branch format: `{type}/issue-{NUMBER}-{slug}` where `{type}` ∈ `feature` (default), `research`, or `fix` — created at **pickup time** (when work actually starts), not when the issue is opened, so the branch forks from current `main` instead of going stale in the backlog. **Claude-authored branches carry a leading `claude/` prefix** (`claude/{type}/issue-{NUMBER}-{slug}`) for provenance; **human-authored branches stay bare**. CI treats the `claude/` prefix as optional and enforces the same shape + issue-existence check either way, at both push time (`enforce-branch-naming`, plus the local `.husky/pre-push` hook) and PR time (`validate-pr-linking`); only the harness-assigned `claude/…` scratch landing-pad branch is skipped. Claude creates the branch itself per `.claude/skills/github-issue-and-branch-workflow/SKILL.md` (the single source of truth for the exact regex).
...
- Workflow: issue created → (when picked up) branch created off current `main` (`claude/{type}/issue-{N}-{slug}` for Claude, bare for humans) → commit/push → PR titled `Closes #{N}` → merge
