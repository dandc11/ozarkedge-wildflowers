...
**The actual run against `prod` processed 26 documents and generated 26
mutations in a single transaction** — one more than the snapshot predicted. The
extra document was `drafts.1ed80cf2-27d8-4db8-a011-2b5a6719e473`, a draft that
did not exist at snapshot time (only its published counterpart did).

Both design decisions earned their keep in that one document:

- Selecting by **GROQ filter rather than a captured list of document IDs** — a
  list frozen from the snapshot would have silently skipped it.
- **Including drafts** — the extra document was itself a draft. Under the usual
  `!(_id in path("drafts.**"))` rule it would have been missed, along with the
  two drafts already known from the snapshot, and "Savannah" would have come
  back the moment any of the three were published.
