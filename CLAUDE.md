...
- Branch format: `{type}/issue-{NUMBER}-{slug}` (`{type}` ∈ `feature` default, `research`, `fix`) — created at **pickup time** (when work actually starts), not when the issue is opened, so the branch forks from current `main` instead of going stale. **Claude-authored branches add a leading `claude/` prefix** for provenance; **human branches stay bare** (CI accepts either). Claude creates the branch itself per `.claude/skills/github-issue-and-branch-workflow/SKILL.md` — the single source of truth for the exact regex and CI behavior.
...
- [Dev dataset (#301)](docs/DEV_DATASET.md)
