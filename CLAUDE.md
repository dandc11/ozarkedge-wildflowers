...
- **Claude Code web/remote sessions & the `claude/` prefix.** Sessions launched from the web/app or a trigger arrive on a harness-assigned scratch branch (e.g. `claude/public-repo-job-search-mndujp`) — that name is generated by the cloud system, not chosen against this convention, and it's just a landing pad. When Claude picks up an issue it creates and works on **`claude/feature/issue-{N}-{slug}`** and opens the PR from that branch. So **Claude-authored PR branches keep the `claude/` prefix** (provenance at a glance), while **human-authored PR branches stay bare `feature/issue-{N}-{slug}`**. Both share the same `feature/issue-{N}-{slug}` shape after any prefix, so issue-number search/sort/archiving stays consistent across the two namespaces.
- **CI treats the prefix as optional.** `validate-pr-linking` accepts `(claude/)?feature/issue-{N}` and still reads `{N}` from the branch to match the `Closes #N` title, so prefixed and bare PR branches both pass. `enforce-branch-naming` skips `claude/**` on push (harness scratch branches never trip it); the `feature/issue-{N}` shape is still enforced at PR time for anything merged to `main`.
